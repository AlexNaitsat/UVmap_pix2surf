...
class Dataset(data.Dataset):
    ...
    def grab(self, meta_info):
        # get direct rgb, nox, mask from NOCS dataset (v_, x_ are _view0, _view1)
        ...
        v_rgb_w_bg = cv.resize(v_rgb_w_bg, self.shape, interpolation=cv.INTER_NEAREST) #resizing to 320x240
        ...
        debug_output_mask = False #My addons
    ...
    #sample forground values (non white) from input 2D image array, returns samples as image vector with shaffled indices
    def get_uv_mask_xyz(self, nocs_map):
        ...
        # # example : output uv channel as an image
        # uv_map_3chanels = np.concatenate([uv_map, np.zeros((uv_map.shape[0], uv_map.shape[1], 1))], axis=2)
        # cv.imwrite('./debug/uv_map.png', 255 * uv_map_3chanels.astype('float32'))

        #convert 'WxHxC' image arrays to 'WHxC' vector images
    ...
    def build_index(self, check=False):
        ...
        for cate in self.cates_list:
            ...
            for obj in models_list:
                ...
                id_set = set(id.split('_')[1] for id in files_list) #set of frame numbers
                for id in id_set:
