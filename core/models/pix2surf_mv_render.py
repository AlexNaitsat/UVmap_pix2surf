    ...
    def __init__(self, cfg):
        ...
        self.output_info_dict = {
            ...
            'image': ['rgb-v', 'uni-rgb-v', 'nox-v-gt'] +
                     ['unwrapped-chart', 'unwrapped-chart-uni' ,'learned-chart', 'sp-image', 'sp-image-uni'] +
...
class Network(MV_Net):
    ...
    def forward(self, pack, is_train=True):
        ...
        for new_scatter in unwrapped_chart_list:
            vis_nsc_uni = torch.max(new_scatter, vis_nsc_uni) # "max" combines unwrapped textures on black bg
        batch['unwrapped-chart-uni'] = vis_nsc_uni            # combined texture from all views in a single image
        ...
        batch['GIM-uni'] = torch.cat(GIM, 3) # 512x2560 image of unwrapped textures of all views
        batch['GIM'] = GIM                   # a list of interpolated unwrapped charts on white bg
