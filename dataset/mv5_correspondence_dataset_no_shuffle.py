...
class Dataset(data.Dataset):
    ...
    def __init__(self, cfg, mode):
        ...
        #ME: moved this block to here to avoid errors inside self.build_index
        self.shape = (320, 240)
        self.n_view = 5              # in dataset/test/cate/ should be 2*self.n_view png images (2 cameras for each view)
        self.supervision_cap = 4096  # how many pixel in nocs map foreground will be sampled to trian SP branch
        # ME: if use noise background augmentation
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            # sampled_list = sample(obj_list, self.n_view) # pick random n_view views
            sampled_list = obj_list[0:self.n_view]  # pick views 0,..., n_view-1 for a simple debugging
        else:
        ...
        for meta_info in sampled_list:            # get image batch data for each of sampled views
            data = self.grab_atom_data(meta_info) # the same as "neurips_dataset.grad", only here all RGB images in [0,1] range
            data_list.append(data)                # add single view item into bathc data list
        for k in data.keys():
        ...
        for view_id in range(self.n_view - 1):
            ...
            crr_idx_mtx.append(idx_list)    # idx_list[idx_list][i] is the list of nearest neighbour of pixel #i in "idx_list+1" view image (i is the pixel index in flattened foreground image)
            crr_mask_mtx.append(mask_list)  # mask_list[idx_list][i] = 0,1 if the nearest neighbour of pixel #i in "idx_list+1" is near enough in XYZ space
            crr_min_d_mtx.append(mind_list) # mask_list[idx_list][i] = is the distance between pixel #i in "idx_list"  view and its nearest neighbour in "idx_list+1" view
            for mask in mask_list:
                pair_count += mask.sum()
        ave_crr_per_view = int(pair_count / self.n_view)
        batch['crr-idx-mtx'] = crr_idx_mtx   # crr_idx_mtx[view_id] = [ nearestNeighIndex[view_id+1], nearestNeighIndex[view_id+2],...,nearestNeighIndex[4] ]
        batch['crr-mask-mtx'] = crr_mask_mtx # same list of lists of pair-view mask of nearest pixel neighbours
    ...
    def grab_atom_data(self, meta_info):
        ...
        debug_output_mask = False #I added debug outputs
        if debug_output_mask:
            mask_v_rgb = cv.cvtColor(mask_v.astype('float32') * 255, cv.COLOR_GRAY2RGB)
            mask_x_rgb = cv.cvtColor(mask_x.astype('float32') * 255, cv.COLOR_GRAY2RGB)

            cv.imwrite('./debug/mask_' + os.path.split(meta_info['view0'])[1], mask_v_rgb)
            cv.imwrite('./debug/mask_' + os.path.split(meta_info['view1'])[1], mask_x_rgb)
            cv.imwrite('./debug/rend_' + os.path.split(meta_info['view1'])[1], x_rgb_x_bg)
            cv.imwrite('./debug/rend_' + os.path.split(meta_info['view0'])[1], v_rgb_w_bg)
    ...
    def get_uv_mask_xyz(self, nocs_map):
        ...
        u = u / len(u) # uv coords are resized to be in [0,1] range
        v = np.arange(0, crop_param[1][1] - crop_param[1][0]) + 0.5
        v = v / len(v)
        col_uv, row_uv = np.meshgrid(v, u)
        uv_map = np.concatenate((row_uv[..., np.newaxis], col_uv[..., np.newaxis]), 2) # WxHx2 tensor of uv map
    ...
    def group_index(self, index):
        """
        the index file is a list of each view sample, group different view of an object
        """
